...
const SkillCard = forwardRef(({ onOpenRecordModal, userId }, ref) => {
  ...
  const fetchSkills = async () => {
    try {
      const res = await fetch(`http://52.78.157.84:8080/skills/${userId}`);
      const data = await res.json();
      if (data.isSuccess && data.result.skills) {
        setSkills(data.result.skills);
      }
    } catch (error) {
      console.error("스킬 데이터 불러오기 실패:", error);
    }
  };
            ...
            <div className="bg-white p-6 rounded-xl border-2 shadow hover:scale-105 transition cursor-pointer w-full">
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{skill.skillName}를 기다리고 있어요</p>
                </div>
                ...
                <div className="w-full space-y-2">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                    <div className="h-2" style={{ width: `${expPercent}%`, backgroundColor: "#61DAFB" }}></div>
                  </div>
                </div>
